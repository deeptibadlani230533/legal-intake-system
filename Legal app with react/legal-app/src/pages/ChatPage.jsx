...
function TypingDots() {
  return (
    <div className="lc-typing">
      {[0, 1, 2].map((i) => (
        <span key={i} className="lc-dot" style={{ animationDelay: `${i * 0.18}s` }} />
      ))}
    </div>
  );
}
...
function Message({ msg, isNew }) {
  ...
  return (
    <div className={`lc-msg-row ${isUser ? "lc-user-row" : "lc-bot-row"} ${isNew ? "lc-msg-enter" : ""}`}>
...
export default function ChatPage() {
  ...
  useEffect(() => {
    const toSave = messages.filter((m) => m.content !== "TYPING");
    try { sessionStorage.setItem(CHAT_STORAGE_KEY, JSON.stringify(toSave)); } catch {}
  }, [messages]);
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    const withTyping = [...withUser, { role: "assistant", content: "TYPING" }];
    setMessages(withTyping);
    setNewMsgIndex(withUser.length - 1);
    setLoading(true);

    try {
      const res = await axios.post(`${API_BASE}/api/chat`, { question: userText });
      const reply = res.data?.response || "I'm sorry, I couldn't retrieve a response. Please try again.";
      const botMsg = { role: "assistant", content: reply };
      const finalMsgs = [...withUser, botMsg];
      setMessages(finalMsgs);
      setNewMsgIndex(finalMsgs.length - 1);
    } catch {
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ambient glows matching Cases page */
        .lc-glow1 {
          position: fixed; top: -120px; right: -120px;
          width: 480px; height: 480px; border-radius: 50%;
          background: radial-gradient(circle, rgba(196,161,88,0.08) 0%, transparent 70%);
          pointer-events: none; z-index: 0;
        }
        .lc-glow2 {
          position: fixed; bottom: -120px; left: -100px;
          width: 440px; height: 440px; border-radius: 50%;
          background: radial-gradient(circle, rgba(28,43,58,0.07) 0%, transparent 70%);
          pointer-events: none; z-index: 0;
        }
        ...
        .lc-header {
          ...
          border-bottom: 1px solid rgba(196,161,88,0.18);
          flex-shrink: 0;
        }

        .lc-header-icon {
          width: 40px; height: 40px; border-radius: 50%;
          background: rgba(196,161,88,0.15);
          border: 1px solid rgba(196,161,88,0.3);
          color: #c4a158;
        ...
        .lc-online-label {
          ...
          color: #4a7c59; letter-spacing: 0.14em;
        ...
        .lc-clear-btn:hover {
          ...
          border-color: rgba(196,161,88,0.3);
          color: #c4a158;
        }
        ...
        .lc-msg-row {
          ...
          animation: lc-fadeup 0.3s ease forwards;
        }
        ...
        .lc-suggestions-label {
          font-size: 9px; font-weight: 600; color: #b8b0a4;
          letter-spacing: 0.18em; text-transform: uppercase;
          margin-bottom: 10px;
          display: flex; align-items: center; gap: 8px;
        }

        .lc-suggestions-label::after {
        ...
        .lc-suggest-chip:hover:not(:disabled) {
          background: #1c2b3a;
          color: #f0ede4;
          border-color: #1c2b3a;
        ...
        /* ── Divider between dates (optional future use) ── */
        .lc-divider {
          display: flex; align-items: center; gap: 12px;
          margin: 12px 0;
          font-size: 10px; color: #c0b9ae; letter-spacing: 0.1em;
          text-transform: uppercase;
        }
      ...
      <div className="lc-root">
        <div className="lc-glow1" />
        <div className="lc-glow2" />
        ...
        <header className="lc-header">
          ...
          <button className="lc-clear-btn" onClick={clearHistory} title="Clear conversation">
            <TrashIcon /> Clear
          </button>
        </header>
