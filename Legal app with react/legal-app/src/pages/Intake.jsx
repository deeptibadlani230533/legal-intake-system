...
import {
  ChevronRight, ChevronLeft, ShieldCheck, Briefcase, IndianRupee, Loader2,
} from "lucide-react";
...
export default function Intake() {
  ...
  const [formData, setFormData] = useState({
    caseTitle: "", description: "", clientName: "",
    clientEmail: "", clientPhone: "", clientAddress: "",
    category: "", incidentDate: "", opponentName: "", claimAmount: "",
  });
  ...
  const validateStep = () => {
    ...
    if (step === 1) {
      ...
      if (!formData.clientEmail.trim())   newErrors.clientEmail = "Email address is required";
      else if (!validateEmail(formData.clientEmail)) newErrors.clientEmail = "Enter a valid email address";
      if (!formData.clientPhone.trim())   newErrors.clientPhone = "Phone number is required";
      else if (!validatePhone(formData.clientPhone)) newErrors.clientPhone = "Enter a valid 10-digit Indian number";
      if (!formData.clientAddress.trim()) newErrors.clientAddress = "Address is required";
    }
    if (step === 2) {
      if (!formData.caseTitle.trim())   newErrors.caseTitle = "Matter title is required";
      if (!formData.category.trim())    newErrors.category = "Please select a legal category";
      if (!formData.incidentDate)       newErrors.incidentDate = "Date of incident is required";
    }
    if (step === 3) {
      if (!formData.opponentName.trim()) newErrors.opponentName = "Opposing party name is required";
      if (!formData.claimAmount)         newErrors.claimAmount = "Claim amount is required";
      if (!formData.description.trim())  newErrors.description = "Case narrative is required";
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) { toast.error(data.message || "Failed to create case."); return; }
  ...
  const stepTitles = [
    { title: "Client Verification",   sub: "Verify the client's personal and contact details." },
    { title: "Case Classification",   sub: "Describe and categorize the legal matter." },
    { title: "Financial & Summary",   sub: "Provide claim details and a case narrative." },
  ];

  const ErrorMsg = ({ field }) =>
    errors[field]
      ? <p style={{ color: "#c0392b", fontSize: 11, marginTop: 5, fontWeight: 400 }}>{errors[field]}</p>
      : null;
          ...
          <div className="it-container">
            ...
            <div className="it-page-header">
              <div>
                ...
                <h1 className="it-page-title">Case <em>Intake</em> Form</h1>
                <p className="it-page-sub">Complete the required information carefully.</p>
              </div>
              ...
              {stepMeta.map((s, i) => (
                <div key={s.n} style={{ display: "contents" }}>
                  <div className="it-step-node">
                    <div className={`it-step-circle ${step > s.n ? "done" : step === s.n ? "active" : ""}`}>
                      {step > s.n ? "✓" : s.n}
                    </div>
                    <span className={`it-step-label ${step > s.n ? "done" : step === s.n ? "active" : ""}`}>
                      {s.label}
                    </span>
                  </div>
                  {i < stepMeta.length - 1 && (
                    <div className={`it-step-connector ${step > s.n ? "done" : ""}`} />
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="it-card">
              ...
              <div className="it-card-header">
                ...
                <div>
                  <div className="it-card-title">{stepTitles[step - 1].title}</div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    {step === 1 && (
                      <>
                        <div className="it-field">
                          ...
                          <Input
                            name="clientName" value={formData.clientName}
                        ...
                        <div className="it-grid-2">
                          <div className="it-field">
                            ...
                            <Input
                              name="clientEmail" value={formData.clientEmail}
                        ...
                        <div className="it-field">
                          ...
                          <Textarea
                            ...
                            onChange={handleChange} placeholder="Full residential address…"
                    ...
                    {step === 2 && (
                      <>
                        <div className="it-field">
                          ...
                          <Input
                            name="caseTitle" value={formData.caseTitle}
                            onChange={handleChange} placeholder="Brief descriptive title of the case"
                        ...
                        <div className="it-grid-2">
                          <div className="it-field">
                            ...
                            <Select
                              ...
                              onValueChange={(v) => { setFormData({ ...formData, category: v }); setErrors({ ...errors, category: "" }); }}
                            >
                              <SelectTrigger className={`it-select-trigger${errors.category ? " error" : ""}`}>
                          ...
                          <div className="it-field">
                            ...
                            <Input
                              type="date" name="incidentDate" value={formData.incidentDate}
                    ...
                    {step === 3 && (
                      <>
                        <div className="it-field">
                          <label className="it-label">Opposing Party Name</label>
                          <Input
                            name="opponentName" value={formData.opponentName}
                            onChange={handleChange} placeholder="Full name of opposing party"
                        ...
                        <div className="it-field">
                          ...
                          <Input
                            type="number" name="claimAmount" value={formData.claimAmount}
